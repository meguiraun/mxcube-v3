...
var SampleTree = React.createClass({displayName: "SampleTree",
    	...
    	that = this//for fixing namespace issue in the next linked loops
 		...
 		return React.createElement("div", {className: "panel panel-default"}, 
              		React.createElement("div", {className: "panel-heading clearfix"}, 
            			   React.createElement("b", {className: "panel-title pull-left"}, "Sample Tree")
                  ), 
                  ...
                  React.createElement("div", {className: "panel-body"}, 
                    React.createElement("ul", null, 
                      ...
                      return React.createElement("li", null, 
                          React.createElement("input", {type: "checkbox", id: 'smp'+item['sample']+'Checked', onChange: that.toggleSelected}), 
                          React.createElement("label", null, item['sample']), 
                          React.createElement("button", {type: "button", className: "btn btn-success btn-xs", id: item['sample'], onClick: that.anAction}, " 'delete'"), 
                             React.createElement("ul", null, 
                              item['nodes'].map(function (node){
                              return React.createElement("li", null, 
                                  React.createElement("input", {type: "checkbox", id: 'col'+item['sample']+node['name']+'Checked', onChange: that.toggleSelected}), 
                                  React.createElement("label", null, node['name']), 
                                  React.createElement("button", {type: "button", id: item['sample']+node, onClick: that.removeNodeFromSample.bind(that,item['sample'],node)}, "'Remove' "), 
                                  React.createElement("button", {type: "button", id: item['sample']+node+'Info', onClick: that.showDetails.bind(that,item['sample'], node)}, "'Show Details' ")
                                 )
                          })
                          )); })
                        
                      ), 
                  React.createElement("button", {onClick: this.runCollection}, "Run queue")
                  )
            )
	    },	
	});

React.render(React.createElement(SampleTree, null), document.getElementById('SampleTreeGoesHere'));
